...
class AdminUserService:
    ...
    def _serialize_user(self, user: User) -> dict:
        ...
        company_ids = [int(x.company_id) for x in self.db.query(UserCompanyAccess).filter(UserCompanyAccess.user_id == user.id).all()]
        country_ids = [int(x.country_id) for x in self.db.query(UserCountryAccess).filter(UserCountryAccess.user_id == user.id).all()]
        region_ids = [int(x.region_id) for x in self.db.query(UserRegionAccess).filter(UserRegionAccess.user_id == user.id).all()]
        branch_ids = [int(x.branch_id) for x in self.db.query(UserBranchAccess).filter(UserBranchAccess.user_id == user.id).all()]
        department_ids = [int(x.department_id) for x in self.db.query(UserDepartmentAccess).filter(UserDepartmentAccess.user_id == user.id).all()]
        team_ids = [int(x.team_id) for x in self.db.query(UserTeamAccess).filter(UserTeamAccess.user_id == user.id).all()]
        customer_ids = [str(x.customer_id) for x in self.db.query(UserCustomerAccess).filter(UserCustomerAccess.user_id == user.id).all()]
    ...
    def update_user_scopes(self, user_id: int, payload: AdminUserScopeUpdateRequest):
        ...
        for company_id in payload.company_ids:
            self.db.add(UserCompanyAccess(user_id=user_id, company_id=company_id, granted_by=None))

        for country_id in payload.country_ids:
            self.db.add(UserCountryAccess(user_id=user_id, country_id=country_id, granted_by=None))

        for region_id in payload.region_ids:
            self.db.add(UserRegionAccess(user_id=user_id, region_id=region_id, granted_by=None))

        for branch_id in payload.branch_ids:
            self.db.add(UserBranchAccess(user_id=user_id, branch_id=branch_id, granted_by=None))

        for department_id in payload.department_ids:
            self.db.add(UserDepartmentAccess(user_id=user_id, department_id=department_id, granted_by=None))

        for team_id in payload.team_ids:
            self.db.add(UserTeamAccess(user_id=user_id, team_id=team_id, granted_by=None))

        for customer_id in payload.customer_ids:
            self.db.add(UserCustomerAccess(user_id=user_id, customer_id=customer_id, granted_by=None))
